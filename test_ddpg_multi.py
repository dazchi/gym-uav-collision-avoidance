...
from pytorch_ddpg_temp.ddpg import DDPG
from pytorch_ddpg_temp.replay_memory import ReplayMemory
...
for i in range(NUM_AGENTS):
    agents.append(DDPG(n_observations, n_actions))
    ...
    for steps in range(MAX_EPISOED_STEPS):
        ...
        if len(memory) > BATCH_SIZE and not EVALUATE:
            ...
            for i in range(UPDATE_PER_STEP):
                ...
                actor_loss, critic_loss = agents[0].update_parameters(memory, BATCH_SIZE)
                for i in range(1, NUM_AGENTS):                                
                    agents[i]._hard_update(agents[0].actor, agents[i].actor)
                tb_writer.add_scalar('loss/critic', critic_loss, updates)
                tb_writer.add_scalar('loss/actor', actor_loss, updates)                
                updates += 1
        ...
        if math.isnan(score):
            ...
            for i in range(NUM_AGENTS):
                ...
                print(old_states)
                print(actions[i])
                print(converted_actions[i])
                print(env.agent_list[i])
                print(env.agent_list[i].location)
                print(env.agent_list[i].done)
                print(env.agent_list[i].velocity)
                print(env.agent_list[i].velocity_prev)
