...
from pytorch_td3_temp.td3 import TD3
...
for i in range(NUM_AGENTS):
    agents.append(TD3(n_observations, n_actions))
    ...
    for steps in range(MAX_EPISOED_STEPS):
        ...
        if len(memory) > BATCH_SIZE and not EVALUATE:
            ...
            for i in range(UPDATE_PER_STEP):
                ...
                agents[0].update_parameters(memory, BATCH_SIZE, updates)
                for i in range(1, NUM_AGENTS):            
                    agents[i].actor.load_state_dict(agents[0].actor.state_dict())              
                updates += 1
